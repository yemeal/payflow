...
class OutboxMessage(BaseModel):
    ...
    конверта (то, что участник видит как commandType / event_type: "order.created").
    id - идентификатор строки outbox; идентификатор самого сообщения
    (commandId / event_id) живёт внутри payload-конверта.

    order_service шлёт только события, но схема таблицы едина для всех сервисов:
    relay generic и одинаков везде (в оркестраторе те же поля).
    """
