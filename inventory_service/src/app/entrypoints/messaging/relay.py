"""
Relay outbox склада: отдельный процесс, публикует PENDING-события в orders.events.

Гарантия ADR-002: событие уходит в шину тогда и только тогда, когда движение
остатков закоммичено. Прямая публикация из обработчика команды (dual write)
теряет событие при падении между commit'ом БД и send'ом в Kafka.

Собирается фабрикой, состояние живёт в main(); глобалей уровня модуля нет.
"""
...
async def main() -> None:
    ...
    try:
        ...
        # graceful: relay дообрабатывает текущий батч и выходит из цикла
        relay.stop()
        stop_task.cancel()
        try:
