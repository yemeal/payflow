"""
Relay единого outbox (ADR-006): публикует PENDING-записи (команды и события)
в брокер. Топик и ключ берутся из самой записи - у оркестратора адресатов много.
Механика перенесена из payment_service/order_service и обобщена.
"""
...
class OutboxRelayService:
    ...
    async def _process_batch(self, batch_size: int) -> None:
        # новый scope на каждую пачку - свежая транзакция
            ...
            async with scope.uow:
                ...
                for message in messages:
                    try:
                        ...
                        # запись считается "ядовитой" и помечается FAILED,
                        ...
                        # упавшее (сохранение порядка публикации в пределах ключа)
                        break
